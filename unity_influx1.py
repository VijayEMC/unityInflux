...
response = newUnity._getMsg(newUnity._doGet("/api/types/metric/instances?fields=name,path,isRealtimeAvailable,description,unitDisplayString,path"))
...
for i in entryObj:    
    print(str(i['id']) + ": " + i['name'] + ": " + i['description'] + ": " + i['path'] + ": Real Time Avail? " + str(i['isRealtimeAvailable']))
    #paths.append(i['path'])
